...
### Research design
Our research will be based on materials from parallel Japanese-English corpora at [https://context.reverso.net/](https://context.reverso.net/) and [http://www.manythings.org/corpus/](http://www.manythings.org/corpus/). We will also use data from [https://www5.atwiki.jp/hmiku/](https://www5.atwiki.jp/hmiku/) lyrics database for more precise statistical testing.
...
We intend to use logistic regression to check whether the mentioned effects are statistically significant and whether they have any intrinsic correlation between each other.
...
1. __inf__ _[character string]_ -- verb in its infinitive form (we decided to preserve it, because there are a few cases of homonymy);
...
5. __part_of_speech__ _[factor]_ -- the verb’s grammatical type of speech:
  ...
  (Note that the division is purely empirical: we assigned labels based on the word's behavious; more on this later in the paper)
...
10. __form__ __[factor]__ -- type of the strategy:
...
```{r}
data.unamb_cont %>%
  lrm(form ~ context_pos + sex + part_of_speech + length + conj + past_tense, data = .) %>%
  print()
```
...
```{r}
data.unamb_cont %>%
  lrm(form ~ context_pos + conj + sex + length + part_of_speech + prov_cond + quest, data = .) %>%
  print()
```
